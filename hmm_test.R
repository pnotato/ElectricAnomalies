...
# Filter data
df_filtered <- subset(df, year(Date) >= 2006 & year(Date) <= 2010 & wday(Date) == 1)
...
df_train <- subset(df_filtered, year(Date) <= 2009)
df_test <- subset(df_filtered, year(Date) == 2010)

# load model checkpoint
fitModel <- readRDS(file = "fitted_hmm_model.rds")
nstates <- fitModel@nstates

# get log likelihood on test data
result <- forwardbackward(fitModel)$logLike
